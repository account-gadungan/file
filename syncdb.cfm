...
<cfparam name="url.source" default="dbhr55test">
<cfparam name="url.dbsrc" default="MSSQLSERVER">
<cfparam name="url.target" default="dbhr55empty0">
<cfparam name="url.dbdst" default="MSSQLSERVER">
<cfoutput>
<!--- target --->
...
<cfif UCase(url.dbsrc) eq "ORACLE">
...
<cfelse><!--- if UCase(url.dbsrc) eq "MSSQLSERVER" --->
	<cfquery name="qTable1" datasource="#SourceDSN#">
...
--->
        ...
        <cfif Ucase(url.dbdst) eq "ORACLE">
...
<cfelse><!--- if UCase(url.dbdst) eq "MSSQLSERVER" --->
	<cfquery name="qTable2" datasource="#TargetDSN#">
		SELECT * FROM Information_Schema.TABLES 
		where table_type = 'BASE TABLE'
		...
		<cfif not isdefined("oldonly")>
			...
			<cfelse><!--- if UCase(url.dbsrc) eq "MSSQLSERVER" --->
 			...
 			<table border="1" width="500"> 
 				<tr><td>ADD TABLE</td><td bgcolor="C0C0C0"><b><!--- <font color="##0000ff">#UCase(SourceDSN)#</font>. ---><font color="##ff0000">#TableCheck#</font><!--- &nbsp;(#Ucase(url.dbsrc)#) ---></b></td></tr> 

			<cfif isdefined("alter")>
				<cfif url.dbdst neq "ORACLE">
...
<!--- 				 <tr>
			...
			<cfif isdefined("alter")>
				...
				<cfif colname neq "" and UCase(url.dbdst) eq "ORACLE">
					<cftry>
	...
	<cfelse>
		...
		<!--- <cfif idencol neq "">
			...
			<cfelseif UCase(url.dbdst) eq "MSSQLSERVER">
				<cftry>
					<cfquery name="qIdenCheckTgt" datasource="#TargetDSN#">
						SELECT TOP 1 IDENTITYCOL
						From "#UCase(TableCheck)#"
		...
		<cfif qColumns2.RecordCount>
 			<table border="1" width="500">
 				<tr><td>ADD COLUMN</td><td bgcolor="C0C0C0"><b><!--- <font color="##0000ff">#UCase(SourceDSN)#</font>. --->#TableCheck#<!--- &nbsp;(#Ucase(url.dbdst)#) ---></b></td></tr> 
			<cfloop query="qColumns2">
				<cfif UCase(url.dbsrc) eq "ORACLE">
					<cfset data_length = data_length>
				<cfelse><!--- if UCase(url.dbsrc) eq "MSSQLSERVER" --->
				...
				<cfif isdefined("alter")>
					...
					<tr><td>SCRIPT</td><td>#UCase(TableCheck)#</td>
						...
						ADD <cfif UCase(url.dbdst) eq "ORACLE">#UCase(column_name)#<cfelse>[#UCase(column_name)#]</cfif> #UCase(Tdata_type)# <cfif NOT ListFindNoCase(NonAtribut,Tdata_Type) AND data_length neq ""><cfif data_length eq "-1"><cfset data_length="MAX"></cfif>(#data_length#)</cfif>			
					</td></tr>
					<cfif isdefined("alteryes")>
					<!--- SCRIPT ADD COLUMN --->
					<cfquery name="qAlter" datasource="#TargetDSN#">
					...
					<cfquery name="qUpdData" datasource="#TargetDSN#">
						...
						SET <cfif UCase(url.dbdst) eq "ORACLE">#UCase(column_name)#<cfelse>[#UCase(column_name)#]</cfif> = '#evaluate("qQry.#column_name#")#'
					</cfquery>
					</cfif>
					 --->
					<!--- end --->

...
do
        ...
        if [ "$DUMP" == "yes" ]; then # If value of DUMP is "yes" then backup database
                ...
                $MYSQLDUMP -CfQq --max-allowed-packet=1G --hex-blob --order-by-primary --single-transaction --routines=true --triggers=true --no-data=false -u $MUSER -p$MPASS -h $MHOST -P $MPORT $db | gzip > $FILE
        fi
done
